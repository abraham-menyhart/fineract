...
public class ExternalAssetOwnersWriteServiceImpl implements ExternalAssetOwnersWriteService {
    ...
    private void validateLoanStatus(Integer loanStatus) {
        if (!ACTIVE_LOAN_STATUSES.contains(LoanStatus.fromInt(loanStatus))) {
    ...
    private Integer fetchLoanStatus(Long loanId) {
        Optional<Integer> loanStatus = loanRepository.findLoanStatusByLoanId(loanId);
        return loanStatus.orElseThrow(() -> new LoanNotFoundException(loanId));
    }
}
